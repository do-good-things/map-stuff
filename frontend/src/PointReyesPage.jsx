...
import { MAPBOX_STYLE, formatDuration, formatDurationShort, formatDate } from './mapConfig';

export default function PointReyesPage() {
  ...
  const [riderVisible, setRiderVisible] = useState({});
  const [rideVisible, setRideVisible] = useState([]);
  ...
  useEffect(() => {
    fetch('/api/tracks/point_reyes')
      ...
      .then(d => {
        setData(d);
        const vis = {};
        Object.keys(d).forEach(r => { vis[r] = true; });
        setRiderVisible(vis);
        const numRides = Math.max(...Object.values(d).map(info => (info.rides || []).length));
        setRideVisible(Array(numRides).fill(true));
        buildTimeline(d);
      });
  }, []);
    ...
    const init = () => {
      ...
      Object.entries(data).forEach(([rider, info]) => {
        (info.rides || []).forEach((coords, rideIdx) => {
          ...
          // Static explore track
          const staticId = `static-${rider}-${rideIdx}`;
          map.addSource(staticId, {
            type: 'geojson',
            data: { type: 'Feature', geometry: { type: 'LineString', coordinates: coords.map(c => [c[1], c[0]]) } },
          });
          map.addLayer({
            id: staticId, type: 'line', source: staticId,
            paint: { 'line-color': info.color, 'line-width': 5, 'line-opacity': 0.9 },
            layout: { 'line-cap': 'round', 'line-join': 'round' },
          });

          // Tooltip on hover
          const popup = new mapboxgl.Popup({ closeButton: false, closeOnClick: false, offset: 10 });
          map.on('mouseenter', staticId, (e) => {
            map.getCanvas().style.cursor = 'pointer';
            const s = info.stats && info.stats[rideIdx];
            if (!s) return;
            const dateStr = formatDate(s.start_time);
            const movStr = s.moving_time_s ? formatDurationShort(s.moving_time_s) : null;
            const avgStr = s.avg_speed_kmh ? s.avg_speed_kmh.toFixed(1) + ' km/h' : null;
            const html = `<div style="font-size:12px;font-family:-apple-system,sans-serif">
          ...
          map.on('mouseleave', staticId, () => {
            map.getCanvas().style.cursor = '';
            popup.remove();
          });

          // Completed (faded) polyline for simulation
          const compId = `completed-${rider}-${rideIdx}`;
          map.addSource(compId, {
            type: 'geojson',
            data: { type: 'Feature', geometry: { type: 'LineString', coordinates: coords.map(c => [c[1], c[0]]) } },
          });
          map.addLayer({
            id: compId, type: 'line', source: compId,
            paint: { 'line-color': info.color, 'line-width': 4, 'line-opacity': 0.35 },
            layout: { 'line-cap': 'round', 'line-join': 'round', visibility: 'none' },
          });
        });

        // Active anim polyline
        ...
        map.addLayer({
          ...
          paint: { 'circle-radius': 5, 'circle-color': info.color, 'circle-stroke-color': '#fff', 'circle-stroke-width': 1.5 },
  ...
  // ── Update static track visibility ──
  useEffect(() => {
    if (!data || !mapRef.current || !sourcesReady.current) return;
    const map = mapRef.current;
    if (!map.isStyleLoaded()) return;

    Object.entries(data).forEach(([rider, info]) => {
      (info.rides || []).forEach((_, rideIdx) => {
        const staticId = `static-${rider}-${rideIdx}`;
        if (!map.getLayer(staticId)) return;
        const show = !simActive && riderVisible[rider] && rideVisible[rideIdx];
        map.setLayoutProperty(staticId, 'visibility', show ? 'visible' : 'none');
      });
    });
  }, [riderVisible, rideVisible, data, simActive]);

  // ── Toggle simulation mode ──
  function enterSim() {
    setSimActive(true);
    if (mapRef.current && sourcesReady.current && data) {
      Object.entries(data).forEach(([rider, info]) => {
        (info.rides || []).forEach((_, rideIdx) => {
          const staticId = `static-${rider}-${rideIdx}`;
          if (mapRef.current.getLayer(staticId)) mapRef.current.setLayoutProperty(staticId, 'visibility', 'none');
        });
        const animId = `anim-${rider}`;
        if (mapRef.current.getLayer(animId)) mapRef.current.setLayoutProperty(animId, 'visibility', 'visible');
      });
    }
    renderFrame(0);
  }
  ...
  function exitSim() {
    stopPlay();
    setSimActive(false);
    anim.current.globalTime = 0;
    anim.current.lastCameraRide = -1;
    ...
    if (mapRef.current && sourcesReady.current && data) {
      Object.entries(data).forEach(([rider, info]) => {
        (info.rides || []).forEach((_, rideIdx) => {
          const compId = `completed-${rider}-${rideIdx}`;
          if (mapRef.current.getLayer(compId)) mapRef.current.setLayoutProperty(compId, 'visibility', 'none');
        });
        const animId = `anim-${rider}`;
        if (mapRef.current.getLayer(animId)) {
          mapRef.current.setLayoutProperty(animId, 'visibility', 'none');
          mapRef.current.getSource(animId).setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: [] } });
        }
        const dotId = `dot-${rider}`;
        if (mapRef.current.getLayer(dotId)) mapRef.current.setLayoutProperty(dotId, 'visibility', 'none');
      });
      const bounds = new mapboxgl.LngLatBounds();
      Object.values(data).forEach(info => {
        (info.rides || []).forEach(coords => { if (coords) coords.forEach(c => bounds.extend([c[1], c[0]])); });
      });
    ...
    Object.entries(data).forEach(([rider, info]) => {
      ...
      if (coords[coords.length - 1][2] <= rideTime) {
        ...
        if (map.getSource(animId)) {
          map.getSource(animId).setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: coords.map(c => [c[1], c[0]]) } });
        }
        if (map.getSource(dotId)) {
          map.getSource(dotId).setData({ type: 'Feature', geometry: { type: 'Point', coordinates: [last[1], last[0]] } });
      ...
      if (map.getSource(animId)) {
        map.getSource(animId).setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: pts } });
      }
      const lastPt = pts[pts.length - 1];
      if (map.getSource(dotId)) {
        map.getSource(dotId).setData({ type: 'Feature', geometry: { type: 'Point', coordinates: lastPt } });
        map.setLayoutProperty(dotId, 'visibility', 'visible');
      }
    });
    ...
    if (playingRef.current) {
      if (currentRide !== a.lastCameraRide) {
        ...
        Object.values(data).forEach(info => {
          ...
          if (c && c.length && c[0][2] < minT) { minT = c[0][2]; startLat = c[0][0]; startLng = c[0][1]; }
        });
        if (startLng !== null) {
        ...
        Object.keys(data).forEach(rider => {
          ...
          if (map.getLayer(dotId) && map.getLayoutProperty(dotId, 'visibility') === 'visible') {
            ...
            if (src) { try { const gj = src.serialize()?.data; if (gj?.geometry?.coordinates) positions.push(gj.geometry.coordinates); } catch {} }
          }
        });
        if (positions.length) {
  ...
  function updateNowPlaying(currentRide, gt) {
    ...
    const label = gt >= a.totalDuration ? 'complete' : `day ${currentRide + 1} of ${numRides}${dateStr ? ' · ' + dateStr : ''}`;
    if (nowPlayingRef.current) nowPlayingRef.current.textContent = label;
    pillRefs.current.forEach((el, i) => { if (el) el.classList.toggle('active', gt < a.totalDuration && i === currentRide); });
  }
  ...
  function animLoop(ts) {
    ...
    if (a.lastRafTs !== null) {
      ...
      if (a.globalTime >= a.totalDuration) { a.globalTime = a.totalDuration; stopPlay(); }
    }
    a.lastRafTs = ts;
  ...
  function startPlay() {
    const a = anim.current;
    if (a.totalDuration <= 0 || playingRef.current) return;
  ...
  useEffect(() => () => { playingRef.current = false; if (anim.current.rafId) cancelAnimationFrame(anim.current.rafId); }, []);

  const handleReset = useCallback(() => {
    stopPlay();
    const a = anim.current;
    a.globalTime = 0; a.lastCameraRide = -1; a.cameraLat = null; a.cameraLng = null;
    a.finishedRiders = {}; a.lastRenderedRide = -1;
    renderFrame(0);
  }, [renderFrame]);
  ...
  const handleJumpToRide = useCallback((i) => {
    stopPlay();
    const a = anim.current;
    a.globalTime = a.rideOffsets[i] || 0; a.lastCameraRide = -1; a.cameraLat = null; a.cameraLng = null;
    a.finishedRiders = {}; a.lastRenderedRide = -1;
  ...
  const handleScrub = useCallback((e) => {
    stopPlay();
    anim.current.finishedRiders = {}; anim.current.lastRenderedRide = -1;
    renderFrame(parseFloat(e.target.value));
  }, [renderFrame]);

  const toggleAllRiders = useCallback(() => {
    const allOn = Object.values(riderVisible).every(v => v);
    const next = {}; Object.keys(riderVisible).forEach(r => { next[r] = !allOn; });
    setRiderVisible(next);
  }, [riderVisible]);

  const toggleAllRides = useCallback(() => {
    const allOn = rideVisible.every(v => v);
    setRideVisible(rideVisible.map(() => !allOn));
  }, [rideVisible]);
    ...
    <div className="app-layout">
      <div className="sidebar">
        <div className="sidebar-header">
          ...
          <div className="sidebar-subtitle">2025 · coastal rides · 3 days</div>
        </div>
        ...
        <div className="sidebar-section">
          <div className="section-label">
            riders
            {!simActive && (
              <button className="toggle-link" onClick={toggleAllRiders}>
                {Object.values(riderVisible).every(v => v) ? 'unselect all' : 'select all'}
              </button>
            )}
          </div>
          {data && Object.entries(data).map(([rider, info]) =>
            simActive ? (
              ...
              <label key={rider} className="check-row">
                <span className="rider-dot" style={{ background: info.color }} />
                <span className="check-label">{rider}</span>
                <input type="checkbox" className="check-input" checked={riderVisible[rider] || false}
                  onChange={e => setRiderVisible(prev => ({ ...prev, [rider]: e.target.checked }))} />
              </label>
            )
          )}
        </div>
        ...
        {!simActive && (
          <div className="sidebar-section">
            <div className="section-label">
              rides
              <button className="toggle-link" onClick={toggleAllRides}>
                {rideVisible.every(v => v) ? 'unselect all' : 'select all'}
              </button>
            </div>
            {rideVisible.map((vis, i) => (
              <label key={i} className="check-row">
                <span className="check-label">Day {i + 1}</span>
                <input type="checkbox" className="check-input" checked={vis}
                  onChange={e => setRideVisible(prev => prev.map((v, j) => j === i ? e.target.checked : v))} />
              </label>
            ))}
          </div>
        )}
        ...
        <div className="sidebar-section">
          <div className="section-label">
            simulation
            {simActive && <button className="toggle-link" onClick={exitSim}>exit</button>}
          </div>
          {!simActive ? (
            <button className="btn btn-primary" onClick={enterSim} style={{ width: '100%' }}>
              ▶ run simulation
            </button>
          ) : (
            <>
              ...
              <input type="range" className="scrubber" ref={scrubberRef} min={0}
                max={anim.current.totalDuration || 1000} defaultValue={0}
                onMouseDown={stopPlay} onTouchStart={stopPlay} onInput={handleScrub} />
              <div className="btn-group">
                <button className="btn btn-primary" disabled={playingState} onClick={startPlay}>▶ Play</button>
                <button className="btn" disabled={!playingState} onClick={stopPlay}>⏸ Pause</button>
              ...
              <div className="ride-pills">
                {Array.from({ length: numRides }, (_, i) => (
                  <button key={i} className="ride-pill" ref={el => pillRefs.current[i] = el}
                    onClick={() => handleJumpToRide(i)}>Day {i + 1}</button>
                ))}
              </div>
            </>
          )}
